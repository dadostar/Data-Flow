	...
	BrightnessSolutionKernel(KernelParameters parameters) {
        ...
        for (int i=0; i<height; i++){
            for (int j=0; j<width; j++){
                ...
                inImage[ (i-1) * width + j] <== i > 0 && inImage[(i-1) * width + j] == 0 && v == 1 ? 2 : 1;
                inImage[ i * width + j - 1] <== j > 0 && inImage[i * width + j - 1] == 0 && v == 1 ? 2 : 1;
                inImage[ (i+1) * width + j] <== (i + 1) < height && inImage[(i + 1) * width + j] == 0 && v == 1? 2 : 1;
                inImage[ i * width + j + 1] <== (j + 1) < width && inImage[i * width + j + 1] == 0 && v == 1? 2 : 1;
            }
        }
        ...
        for (int i=0; i<height; i++){
            for (int j=0; j<width; j++){
                inImage[i * width + j] <== inImage[i * width + j] == 2? 1 : 0;
                
            }
        }
   
		io.output("outImage", inImage, result.getType());
	}
}
